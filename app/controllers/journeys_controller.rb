class JourneysController < ApplicationController
  ...
  def show
    ...
    @journey.description = @journey.description.gsub(/(?<=<insert_form>).*?(?=<\/insert_form>)/) {|s| render_notification_subscription_component_to_string([s])} if @journey.description.include? '<insert_form>'
