    ...
    context 'type list exists in TYPES' do
      context 'user is assigned to listing' do
        it 'uses user email' do
          expect(SubscribeSendinblueJob).to receive(:perform_later).with('peter@example.com', 'NewsletterSubscription')
      ...
      context 'anonymous user' do
        it 'uses email' do
          expect(SubscribeSendinblueJob).to receive(:perform_later).with('andrej@example.com', 'NewsletterSubscription')
